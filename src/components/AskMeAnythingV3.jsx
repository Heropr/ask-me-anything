...
function AskMeAnythingV3() {
  ...
  return (
    <div className="ama-v3">
      <div className="ama-v3-header">
        <h1 className="ama-v3-title">
          Ask Me <em>Anything</em>
        </h1>
        <p className="ama-v3-subtitle">
          Ask Ailene anything you'd like to know about Smirk
        </p>
      </div>

      <div className="ama-v3-card">
        <div className="ama-v3-main">
          {!hasConversation && (
            <div className="ama-v3-prompt">
              <h2 className="ama-v3-prompt-text">What can I help you with?</h2>
              <div className="ama-v3-chips">
                {suggestionQuestions.map((question, index) => (
                  <button
                    key={index}
                    className="ama-v3-chip"
          ...
          {hasConversation && (
            <div className="ama-v3-conversation" ref={conversationRef}>
              {conversation.map((entry, index) => (
                <div
                  key={index}
                  className={`ama-v3-conversation-entry ${index === activeIndex ? 'active' : ''}`}
                  ref={index === activeIndex ? activeEntryRef : null}
                  onClick={() => handleEntryClick(index)}
                >
                  <div className="ama-v3-question-group">
                    <div className="ama-v3-question-label">Question {index + 1}</div>
                    <div className="ama-v3-question-bubble">{entry.question}</div>
                  </div>
                  <div className="ama-v3-answer">
                    {index === streamingIndex
                      ? entry.answer.split(' ').slice(0, displayedWords).join(' ')
                      : entry.answer
                    }
                    {index === streamingIndex && <span className="ama-v3-cursor" />}
                  </div>
                  <div className="ama-v3-separator" />
                </div>
              ))}
            </div>
          )}
